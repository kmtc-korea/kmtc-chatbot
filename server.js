/* backend/index.js – KMTC AI 2025-06-10 (v7)  
   · GPT “type / cremated” 판단  
   · 공항 좌표 자동 캐시(IATA → Nominatim)  
   · 지오코딩 3-단계(원문→국가명 붙여 재조회)  
   · 출발·도착 주소는 항공/고인 때만 필수
*/
...
import airportsInit from "./data/airports.json" assert {type:"json"};  // 66 MB,  40k 공항
config();
...
/* ────── 주소 → 좌표 3-단계 ────── */
import countries from "./data/countries.json" assert {type:"json"};     // ISO 코드·한글·영문 250개
...
const seg = async (f,t,v)=>{const km=distKm(f,t);return{km:+km.toFixed(0),hr:hr(km,v)}};
async function routeInfo(depLoc,arrLoc,depIata,arrIata){
  ...
  return{
    leg1:await seg(from,depA,50),
    leg2:await seg(depA,arrA,800),
    leg3:await seg(arrA,to,40)
  };
}
...
app.post("/chat",async(req,res)=>{
  ...
  /* 항공·고인만 주소 필수 */
  if(!from||!to){
    return res.json({reply:"📝 \"…에서 …까지\" 형식 또는 출발·도착 주소·병원명을 입력해 주세요."});
  }

  /* 경로 계산 */
  let legs; try{ legs=await routeInfo(from,to,depAirport,arrAirport);}catch{return res.json({reply:"⚠️ 위치 검색 실패. 주소를 다시 입력해 주세요."});}
  const km=legs.leg2.km;

  /* GPT 계획 */
  let plan; try{ plan=await gptPlan(ses.patient||{},km);}catch{return res.json({reply:"⚠️ AI 계획 생성 실패"});}
